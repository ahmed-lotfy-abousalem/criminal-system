...
#include"functions.h"
#include"structs.h"
#include<vector>     //tried using vectors-- instead of dynamic array
using namespace std;
...
void reading()
{
	...
	if (Admin.is_open())
	{
		while ((Admin.peek()) != EOF) //end of the file
		{
			...
			admin_login admin_temp = {
				admin_email,admin_pass,admin_ID
			};
			admin1.push_back(admin_temp);
		}
		Admin.close();
	}
	...
	if (Crime.is_open())
	{
		while (Crime.peek() != EOF)
		{
			...
			crimes[num_of_crimes].criminals_incase = stoi(numofcriminals);
			...
			crimes[num_of_crimes].DAY_MONTH_YEAR.DAY = stoi(day);
			crimes[num_of_crimes].DAY_MONTH_YEAR.month = stoi(month);
			crimes[num_of_crimes].DAY_MONTH_YEAR.year = stoi(year);
...
void begin()
{
	...
	if (num == 1)
	{
		bool verified = adminconfig();

		if (verified == false)
		{
			string adminid;
			cout << "  ID FOR ACCOUNT   ";     cin >> adminid;
			for (int i = 0; i < 10; i++)
			{
				if (adminid == admin1[i].ID)
				{
					cout << " SO WE WILL RESET YOUR PASSWORD" << endl;
					string newpassword;
					cout << "\n  Enter your new password:   ";    cin >> newpassword;
					admin1[i].password = newpassword;
					system("cls");
					output_admin();
					verified = true;
					break;
...
bool adminconfig()
{
	...
	do
	{
		...
		for (int i = 0; i < admin1.size(); i++)
		{
			if ((useremail == admin1[i].username) && (userpassword == admin1[i].password))
			{
...
void output_admin()
{
	...
	do
	{
		cout << "  1-- add new criminal or officer or a crime" << endl ;
		cout << "  2-- update database" << endl ;
		cout << "  3-- search for data" << endl;
		cout << "  4-- display all data" << endl ;
		cout << "  5--- delete data" << endl ;
		cout << "  6--- assign a crime to officer" << endl;
...
void addcrime()
{
	do
	{
		...
		else if (choice == 2)
		{
			...
			cout << endl;
			cout << " Enter the num_of_criminals_involved:               |"; cin >> crimes[num_of_crimes].criminals_incase;
			cout << endl;
			for (int i = 0; i < crimes[num_of_crimes].criminals_incase; i++)
			{
			...
			cout << " Enter the crime day(int):                          |"; cin >> crimes[num_of_crimes].DAY_MONTH_YEAR.DAY;
			cout << endl;
			cout << " Enter the crime month(int):                        |"; cin >> crimes[num_of_crimes].DAY_MONTH_YEAR.month;
			cout << endl;
			cout << " Enter the crime year(int):                         |"; cin >> crimes[num_of_crimes].DAY_MONTH_YEAR.year;
			cout << endl;
			cout << " The date is:" << crimes[num_of_crimes].DAY_MONTH_YEAR.DAY << "/" << crimes[num_of_crimes].DAY_MONTH_YEAR.month << "/" << crimes[num_of_crimes].DAY_MONTH_YEAR.year << endl;
			num_of_crimes++;
		...
		else if (choice == 2)
		{
			...
			else
			{
				...
				case 2:
				{
					...
					crimes[updatecrime].criminals_incase = criminalsinvolved;
					cout << "\t  Data has been modified successfully" << endl;
					break;
				}
				case 3:
				{
					...
					if (number != 0 && number <= crimes[updatecrime].criminals_incase)
					{
				...
				case 6:
				{
					...
					crimes[updatecrime].DAY_MONTH_YEAR.DAY = dayofcrime;
					cout << "\t  Data has been modified successfully" << endl;
					break;
				}
				case 7:
				{
					...
					crimes[updatecrime].DAY_MONTH_YEAR.month = monthofcrime;
					cout << "\t  Data has been modified successfully" << endl;
					break;
				}
				case 8:
				{
					...
					crimes[updatecrime].DAY_MONTH_YEAR.year = yearofcrime;
					cout << "\t  Data has been modified successfully" << endl;
					break;
				}
				default:
...
void displaycrime(int crimeindex)
{
	...
	cout << "  [2].\t" << "Num_of_criminals_involved:  |" << crimes[crimeindex].criminals_incase << endl;
	for (int i = 0; i < crimes[crimeindex].criminals_incase; i++)
	{
	...
	cout << "  [6].\t" << "Crime day:                  |" << crimes[crimeindex].DAY_MONTH_YEAR.DAY << endl;
	cout << "  [7].\t" << "Crime month:                |" << crimes[crimeindex].DAY_MONTH_YEAR.month << endl;
	cout << "  [8].\t" << "Crime year:                 |" << crimes[crimeindex].DAY_MONTH_YEAR.year << endl;
	cout << "\n";
}
			...
			else
			{
				...
				if (e == 'y' || e == 'Y')
				{
					for (int i = index; i < num_of_crimes - 1; i++)
					{
						...
						crimes[i].criminals_incase = crimes[i + 1].criminals_incase;
						for (int j = 0; j < crimes[i].criminals_incase; j++)
						{
						...
						crimes[i].DAY_MONTH_YEAR.DAY = crimes[i + 1].DAY_MONTH_YEAR.DAY;
						crimes[i].DAY_MONTH_YEAR.month = crimes[i + 1].DAY_MONTH_YEAR.month;
						crimes[i].DAY_MONTH_YEAR.year = crimes[i + 1].DAY_MONTH_YEAR.year;
					}
					num_of_crimes--;
					cout << "  Data has been deleted successfully..." << endl;
				}
...
void addcrimetoofficer()
{
	string sofficerid;   int scrimeid;
	int count = 0, cnt = 0;
	do
	{
		cout << "\tDo you want to  [1].View unassigned Crimes ,[2].View free Officers ,[3].Assign a case to an officer???"<<endl;
		cin >> choice;
		while (choice != 1 && choice != 2 && choice != 3)
		{
			cout << "  Invalid option...Please choose from menu...";  cin >> choice;
						...
						for (int j = 0; j < num_of_crimes; j++)
						{
							if (crimes[j].crimeID == scrimeid)
							{
								...
								if (crimes[j].status == "unassigned")
								{
									...
									cout << "\n\CRIME IS SUCCESSFULLY ASSIGNED TO OFFICER MENTIONED\n";
								}
									
								else
								{
									cout << "  Crime UNAVAILABLE\n";
									break;
								}
							}
						}
...
void Write()
{
	...
	if (Admininput.is_open())
	{
		for (int i = 0; i < admin1.size(); i++)
		{
			Adminoutput << admin1[i].username << endl;
			Adminoutput << admin1[i].password << endl;
			Adminoutput << admin1[i].ID << endl;
		}
	}
	...
	if (Crimeinput.is_open())
	{
		for (int i = 0; i < num_of_crimes; i++)
		{
			...
			Crimeoutput << crimes[i].criminals_incase << endl;
			for (int j = 0; j < crimes[i].criminals_incase; j++)
			{
			...
			Crimeoutput << crimes[i].DAY_MONTH_YEAR.DAY << endl;
			Crimeoutput << crimes[i].DAY_MONTH_YEAR.month << endl;
			Crimeoutput << crimes[i].DAY_MONTH_YEAR.year << endl;
		}
	}
}
			...
			else
			{
				...
				for (int i = 0; i < officer[officerindex].num_assignedcases; i++)
				{
					...
					for (int j = 0; j < num_of_crimes; j++)
					{
						...
						{
							cout << "  [2].\t" << "Num_OF_Criminals_Involved:\t|" << crimes[j].criminals_incase << endl;
							for (int k = 0; k < crimes[j].criminals_incase; k++)
							{
							...
							cout << "  [6].\t" << "Crime day:                \t|" << crimes[j].DAY_MONTH_YEAR.DAY << endl;
							cout << "  [7].\t" << "Crime month:              \t|" << crimes[j].DAY_MONTH_YEAR.month << endl;
							cout << "  [8].\t" << "Crime year:               \t|" << crimes[j].DAY_MONTH_YEAR.year << endl;
						}
					}
		...
		case 3:
		{
			...
			if (found)
			{
				...
				case 2:
				{
					...
					crimes[crimeindex].criminals_incase = newnumofcriminals;
					cout << "  Data has been modified successfully" << endl;
					break;
				}
				case 3:
				{
					int number;
					cout << "  Please enter the order of criminal ID that you want to modify:";  cin >> number;
					if (number != 0 && number <= crimes[crimeindex].criminals_incase)
					{
